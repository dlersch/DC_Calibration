...
public class calibFnToDraw_withGROOT extends Func1D {
	...
	//docaMax comes from 2*wpdist[SL-1], but eventually I want to make it CCDB dependent
	//    by making the main program CCDB dependent and getting its value as input par.
	//    that's why I chose it as one of the input pars, although I could derive it from SL.
	...
	*/  
	...
	for(int loop = 0; loop < pars.size(); loop++){
	    this.addParameter(pars.get(loop)); //This is mandatory, else we get java.lang.IndexOutOfBoundsException
	    //this.setParameter(loop,prevFitPars[loop]); //Works, but this.setParameters(prevFitPars); below also works
	}
	this.setParameters(prevFitPars);   //Valid for Func1D (groot) (see following link)
	...
	//Many lines copied from calcTimeFunc(..) function in the main class file
	double Dc = docaMax*cos30; //docaMax = 2*wpdist[SL-1], 

	
	//double X=docaByDocaMax, x=X*dMax, Xhat0 = X/cos30;
	double x = xNorm*docaMax; //argument xNorm is actually x/docaMax, below we want real x
	...
	double v0Par = v0, tMax = tmax1;  if(SL==2) tMax = tmax2;
	...
	//8/3/16 ===== additing new contribution from beta-dependent time walk (with 5th par 'distbeta')
	//===================== 8/3/16
